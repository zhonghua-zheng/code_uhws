...
def XGB_test(df,year,lat,lon,member):
    ...
    XGBreg = pickle.load(open("/glade/scratch/zhonghua/UrbanHWs/ensem_model/"+year+"/"+"MX_"+lat+"_"+lon+".dat","rb"))
...
for lat in lat_lon_dict:
    ...
    for lon in lat_lon_dict[lat]:
        ...
        if (i%10==0):
            print(i)
pd.concat(df_final_ls).to_csv("/glade/scratch/zhonghua/UrbanHWs/CESM_valid_"+start_date+"/"+member+"_ens.csv")
